...
import { createResourceEditorResolver } from "../src/pages";
import type {
...
const pageTypeId = qualifyContributionId(
	owner,
	"page",
	"viewer",
) as ResourceEditorRegistration["pageTypeId"];
const resource = {
	canonicalId: "asset:one",
	uri: "file:///game/hero.glb",
	kind: "mesh",
};

function editor(
	localId: string,
	overrides: Partial<ResourceEditorRegistration> = {},
): ResourceEditorRegistration {
	return {
...
describe("ResourceEditorResolver", () => {
	...
	it("orders by source layer, priority, then stable id", () => {
		const contributions =
			createContributionRegistry<PagePlatformContribution>();
		const resolver = createResourceEditorResolver(
			contributions,
			{} as PagePort,
		);
		const optionalUser = editor("z-user", { sourceLayer: "user" });
		const defaultProject = editor("a-project", {
			sourceLayer: "project",
			priority: "default",
		});
		const defaultInstalled = editor("b-installed", { priority: "default" });
		contributions.contribute(owner, {
			resourceEditors: [defaultInstalled, defaultProject, optionalUser],
		});
	...
	it("reserves the fallback tier for resources no targeted editor claims", () => {
		...
		const fallback = editor("default-view", {
		...
		const undeclared = {
			canonicalId: "asset:two",
			uri: "forgeax-asset://two",
	...
	it("lets a user association lift the default editor above a targeted one", () => {
		...
		const targeted = editor("mesh-view", {
			selector: { kinds: ["mesh"] },
			priority: "default",
			sourceLayer: "builtin",
		});
		...
		const pages = {
			open: async (request: unknown) => {
				...
				return {
					...
					typeId: pageTypeId,
					resourceId: resource.canonicalId,
				};
			},
		} as PagePort;
