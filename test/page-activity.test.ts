...
import { createActivityRegistry } from "../src/pages";
import type { PagePlatformContribution, PagePort } from "../src/pages";
...
describe("ActivityRegistry", () => {
	...
	it("derives ordered launchers and routes them through PagePort", async () => {
		...
		const opened: string[] = [];
		const pages = {
			open: async ({ typeId }: { typeId: string }) => {
				opened.push(typeId);
				return { cardinality: "singleton", typeId };
			},
		} as PagePort;
		const registry = createActivityRegistry(
			contributions,
		...
		contributions.contribute(owner, {
			activities: [{ id: activityId, title: "Main", order: 2, pageTypeId }],
		});
	...
	it("sorts by sourceLayer first, then order (unranked plugins fall to layer middle, never the front)", () => {
		...
		const mk = (local: string) =>
			qualifyContributionId(owner, "activity", local);
