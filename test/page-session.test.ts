...
import { createContributionRegistry } from "@forgeax/extension-platform/platform";
import { createPageRegistry } from "../src/pages";
import { createPageSession } from "../src/pages";
import type {
	PageController,
...
function setup(pageTypes: readonly PageTypeRegistration[]) {
	...
	const session = createPageSession(registry, {
		project: { getCurrentProject: () => "default", subscribeCurrentProject: () => () => {} },
		createInstanceId: () => "generated",
		now: () => 42,
	});
...
function page(
	id: string,
	cardinality: "singleton" | "resource" | "multi-instance",
...
describe("PageSession", () => {
	it("deduplicates singleton opens and focuses the existing instance", async () => {
		...
		expect(session.getSnapshot().instances[0]?.context).toEqual({
			source: "first",
		});
	...
	it("requires an explicit save/discard decision for dirty pages", async () => {
		...
		const controller: PageController = {
			prepareClose: () =>
				dirty ? { status: "dirty", message: "Unsaved" } : { status: "ready" },
			save: () => {
				dirty = false;
			},
			discard: () => {
		...
		await expect(session.close(key)).rejects.toMatchObject({
			code: "PAGE_CLOSE_REQUIRES_DECISION",
		});
		expect(session.getSnapshot().instances).toHaveLength(1);
	...
	it("reflects a controller reactive title into the snapshot and cleans it up on close", async () => {
		...
		const closableSingleton: PageTypeRegistration = {
			...page(singletonId, "singleton", controller),
			closable: true,
		};
	...
	it("delegates context-menu items to the page controller and evaluates them fresh", async () => {
		...
		expect(session.getContextMenuItems(key).map((i) => i.id)).toEqual([
			"copy",
			"save",
		]);
		expect(
			session.getContextMenuItems(key).find((i) => i.id === "save")?.disabled,
		).toBe(true);
		dirty = true;
		expect(
			session.getContextMenuItems(key).find((i) => i.id === "save")?.disabled,
	...
	it("preflights every owned page before an extension cleanup removes any", async () => {
		...
		const ready: PageController = {
			prepareClose: () => ({ status: "ready" }),
			dispose: () => undefined,
		};
		const dirty: PageController = {
			prepareClose: () => ({ status: "dirty" }),
			dispose: () => undefined,
		};
		const { session } = setup([
		...
		await expect(session.closeOwnedBy(owner)).rejects.toMatchObject({
			code: "PAGE_CLOSE_REQUIRES_DECISION",
			name: "PageClosePreparationDeferredError",
		});
		expect(session.getSnapshot().instances).toHaveLength(2);
	});
});

beforeEach(() => { GlobalRegistrator.register(); localStorage.clear(); });
