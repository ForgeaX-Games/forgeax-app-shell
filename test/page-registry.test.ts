...
import type { PagePlatformContribution } from "../src/pages";
...
describe("PageRegistry", () => {
	it("resolves required panels and prunes missing optional placements from layout", () => {
		const contributions =
			createContributionRegistry<PagePlatformContribution<never>>();
		const registry = createPageRegistry(contributions);
		contributions.contribute(owner, {
			panelTypes: [
				{ id: mainPanelId, runtime: { kind: "inline", render: () => null } },
			],
			pageTypes: [page],
		});
		...
		expect(resolved.layout.root).toEqual({
			kind: "tabs",
			placements: ["main"],
		});
	});
	...
	it("rejects conflicts and owner/id mismatches before publication", () => {
		...
		const contribution = {
			panelTypes: [
				{
					id: mainPanelId,
					runtime: { kind: "inline" as const, render: () => null },
				},
			],
			pageTypes: [page],
		};
		...
		expect(() => registry.validateContribution(owner, contribution)).toThrow(
			"already registered",
		);
		expect(() =>
			registry.validateContribution("@forgeax-plugin/other", {
				pageTypes: [page],
			}),
	...
	it("fails staging when a required panel reference is unresolved", () => {
		...
		const registry = createPageRegistry(contributions);
		expect(() =>
			registry.validateContribution(owner, { pageTypes: [page] }),
		).toThrow("requires missing panel type");
