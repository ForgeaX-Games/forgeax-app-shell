...
export type KeybindingResolution =
  | {
      ...
      readonly reason: 'editable' | 'composition';
    }
  | { readonly status: 'unclaimed' }
  | {
      readonly status: 'claimed-disabled';
      readonly binding: RegisteredKeybinding;
    }
...
export function normalizeKeybinding(
  ...
  const ordered = ['Ctrl', 'Meta', 'Alt', 'Shift'].filter((modifier) =>
...
export function normalizeKeyboardEvent(
  event: KeyboardEvent,
  _platform: KeybindingPlatform = detectKeybindingPlatform(),
): NormalizedKeyEvent {
  const modifiers = [
    event.ctrlKey ? 'Ctrl' : '',
    event.metaKey ? 'Meta' : '',
...
export function resolveKeybinding(
  ...
  for (const scope of scopeOrder) {
    const candidates = input.bindings
      ...
      .filter((binding) =>
        bindingMatches(binding, input.normalized, input.platform),
      )
      .filter((binding) => !binding.when || binding.when(context))
      .filter(
        (binding) => !input.normalized.editable || binding.allowInEditable,
      )
      .filter(
        (binding) =>
          !input.normalized.composing || binding.allowDuringComposition,
      )
      .sort(
        (a, b) =>
  ...
  if (input.normalized.composing)
    return { status: 'passthrough', reason: 'composition' };
  if (input.normalized.editable)
    return { status: 'passthrough', reason: 'editable' };
