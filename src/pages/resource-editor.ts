...
function matches(
	editor: ResourceEditorRegistration,
	resource: ResourceDescriptor,
): boolean {
	const selector = editor.selector;
	let scheme: string | undefined;
	try {
		scheme = new URL(resource.uri).protocol.replace(/:$/u, "");
	} catch {
		/* path-like URI */
	}
	const path =
		(resource.displayPath ?? resource.uri).split(/[?#]/u, 1)[0] ?? "";
...
function byRank(
	a: ResourceEditorRegistration,
	b: ResourceEditorRegistration,
): number {
	return (
		layerRank[b.sourceLayer ?? "installed"] -
			layerRank[a.sourceLayer ?? "installed"] ||
...
function readAssociations(): Record<string, QualifiedResourceEditorId> {
	try {
		...
		return parsed && typeof parsed === "object"
			? (parsed as Record<string, QualifiedResourceEditorId>)
			: {};
	} catch {
		return {};
	}
...
function writeAssociations(
	value: Readonly<Record<string, QualifiedResourceEditorId>>,
): void {
	try {
...
export function createResourceEditorResolver(
	...
	const sortedCandidates = (
		...
		const all = contributions
			.entries()
			.flatMap(({ item }) => item.resourceEditors ?? []);
		...
		const targeted = all
			.filter((editor) => matches(editor, resource))
			.sort(byRank);
		const fallbacks = all
			.filter((editor) => editor.selector.fallback === true)
			.sort(byRank);
		...
		return [...targeted, ...fallbacks].sort(
