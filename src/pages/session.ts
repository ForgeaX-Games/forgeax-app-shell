import { decodePageKey, encodePageKey, type PageKey } from "@forgeax/types/page";
import type {
	PageCloseDecision,
	PageCloseReason,
	PageController,
...
export class PageClosePreparationDeferredError extends PagePlatformError {
	constructor(
		code: "PAGE_CLOSE_VETOED" | "PAGE_CLOSE_REQUIRES_DECISION",
		message: string,
...
export interface PageProjectContext {
	...
	subscribeCurrentProject(listener: (projectId: string) => void): () => void;
}

export interface CreatePageSessionOptions {
	readonly project: PageProjectContext;
...
export function createPageSession(
	...
	const createInstanceId =
		options.createInstanceId ?? (() => crypto.randomUUID());
	...
	const recentKey = (policy: "session" | "project"): string =>
		policy === "project"
			? `forgeax:project:${options.project.getCurrentProject()}:recent-page`
	...
	const persistRecent = (): void => {
		...
		const request: PageOpenRequest = {
			...
			...(Object.keys(entry.instance.context).length > 0
				? { context: entry.instance.context }
				: {}),
		};
		try {
			localStorage.setItem(recentKey(policy), JSON.stringify(request));
		} catch {
			/* quota/private mode */
		}
	};
	...
	const publish = (): void => {
		...
		snapshot = {
			...
			instances: [...entries.entries()].map(([encoded, entry]) =>
				titles.has(encoded)
					? { ...entry.instance, title: titles.get(encoded) as string }
					: entry.instance,
			),
		};
	...
	const bindTitle = (encoded: string, controller: PageController): void => {
		...
		if (controller.subscribeTitle) {
			titleSubs.set(
				encoded,
				controller.subscribeTitle(() => read()),
			);
		}
	};
	...
	const requireEntry = (
		key: PageKey | string,
	): { encoded: string; entry: SessionEntry } => {
		...
		if (!entry) {
			throw new PagePlatformError(
				...
				`page instance "${encoded}" is not open`,
				{ key: encoded },
			);
		}
	...
	const resolvePreparation = async (
		...
		if (preparation.status === "vetoed") {
			throw new PageClosePreparationDeferredError(
				"PAGE_CLOSE_VETOED",
				preparation.message ?? "page refused to close",
				{
		...
		if (decision === "save") {
			if (!entry.controller.save) {
				throw new PageClosePreparationDeferredError(
					"PAGE_CLOSE_VETOED",
					"page cannot save its dirty state",
					{
						key: entry.instance.encodedKey,
						reason,
					},
			...
			if (!entry.controller.discard) {
				throw new PageClosePreparationDeferredError(
					"PAGE_CLOSE_VETOED",
					"page cannot discard its dirty state",
					{
						key: entry.instance.encodedKey,
						reason,
					},
	...
	const session: PageSession = {
		async open(request: PageOpenRequest): Promise<PageKey> {
			...
			if (!resolved) {
				throw new PagePlatformError(
					"PAGE_TYPE_NOT_FOUND",
					`page type "${request.typeId}" is not registered`,
					{
			...
			if (resolved.status === "unavailable") {
				throw new PagePlatformError(
					...
					`page type "${request.typeId}" is unavailable`,
					{
						typeId: request.typeId,
			...
			if (resolved.definition.cardinality === "singleton") {
				if (request.resource || request.instanceId) {
					throw new PagePlatformError(
						...
						"singleton page does not accept resource or instance identity",
					);
				}
				...
				if (!request.resource || request.instanceId) {
					throw new PagePlatformError(
						"PAGE_CARDINALITY_MISMATCH",
						"resource page requires exactly one resource identity",
					);
				}
				key = {
					cardinality: "resource",
					typeId: request.typeId,
					resourceId: request.resource.canonicalId,
				};
			} else {
				if (request.resource) {
					throw new PagePlatformError(
						"PAGE_CARDINALITY_MISMATCH",
						"multi-instance page does not accept a resource identity",
					);
				}
				key = {
			...
			const controller =
				(await resolved.definition.createController?.({
					key,
					context,
		...
		reorder(key, toIndex): void {
			...
			const moved = order.map(
				(k) => [k, entries.get(k) as SessionEntry] as const,
			);
			entries.clear();
		...
		async close(key, request = {}): Promise<void> {
			...
			if (!entry.instance.closable && reason === "user") {
				throw new PagePlatformError(
					"PAGE_CLOSE_VETOED",
					"page is permanent and cannot be closed",
					{ key: encoded },
				);
			}
		...
		async closeOwnedBy(
			...
			const owned = [...entries.values()].filter(
				(entry) => registry.ownerOf(entry.instance.typeId) === owner,
			);
			for (const entry of owned)
				await resolvePreparation(entry, reason, decision);
			for (const entry of owned) await entry.controller.dispose();
			if (owned.length === 0) return;
			const ownedKeys = new Set(
				owned.map((entry) => entry.instance.encodedKey),
			);
			for (const key of ownedKeys) {
				entries.delete(key);
				unbindTitle(key);
			}
			if (activeKey && ownedKeys.has(activeKey))
				activeKey = [...entries.keys()].at(-1);
			publish();
		},
		...
		async dispose(): Promise<void> {
			...
			for (const entry of all)
				await resolvePreparation(entry, "host-dispose", "discard");
	...
	const tryRestoreRecent = async (): Promise<void> => {
		if (restoring || entries.size > 0 || typeof localStorage === "undefined")
			return;
		for (const policy of ["project", "session"] as const) {
			...
			try {
				request = JSON.parse(raw) as PageOpenRequest;
			} catch {
			...
			if (
				...
				(resolved.definition.restorePolicy ?? "never") !== policy
			) {
				localStorage.removeItem(key);
				continue;
			}
	...
	projectUnsubscribe = options.project.subscribeCurrentProject(() => {
		void tryRestoreRecent();
	});
	void tryRestoreRecent();
