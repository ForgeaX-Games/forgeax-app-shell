...
export interface PageController {
	...
	): PageClosePreparation | Promise<PageClosePreparation>;
...
export interface PageTypeRegistration<LegacyLayout = unknown> {
	...
	readonly layout: PageLayoutEnvelope | LegacyLayout;
	readonly panels: readonly PagePanelPlacement[];
	readonly createController?: (
		context: PageControllerContext,
	) => PageController | Promise<PageController>;
}

export interface PagePlatformContribution<LegacyLayout = unknown> {
	readonly pageTypes?: readonly PageTypeRegistration<LegacyLayout>[];
...
export interface ResourceEditorResolver {
	...
	setUserAssociation(
		...
		editorId: QualifiedResourceEditorId | null,
	): void;
}
...
export type ResolvedPageType<LegacyLayout = unknown> =
	| {
			readonly status: "available";
			readonly owner: string;
			readonly definition: PageTypeRegistration<LegacyLayout>;
			readonly layout: PageLayoutEnvelope | LegacyLayout;
...
export interface PageRegistrySnapshot<LegacyLayout = unknown> {
	readonly generation: number;
	readonly pageTypes: ReadonlyMap<QualifiedPageTypeId, ResolvedPageType<LegacyLayout>>;
	readonly panelTypes: ReadonlyMap<QualifiedPanelTypeId, PanelTypeRegistration>;
}

export interface PageRegistry<LegacyLayout = unknown> {
	get(typeId: QualifiedPageTypeId): ResolvedPageType<LegacyLayout> | undefined;
	ownerOf(typeId: QualifiedPageTypeId): string | undefined;
	getSnapshot(): PageRegistrySnapshot<LegacyLayout>;
	subscribe(listener: () => void): () => void;
...
export type { PageCardinality, PageErrorCode, PageKey, PageLayoutEnvelope, QualifiedActivityId, QualifiedPageTypeId, QualifiedPanelTypeId, QualifiedResourceEditorId, ResourceDescriptor, ResourceSelector } from "@forgeax/types/page";
