...
function sanitizeLayoutNode(
	...
	if (node.kind === "tabs") {
		const placements = node.placements.filter((placement) =>
			allowed.has(placement),
		);
		if (placements.length === 0) return undefined;
		return {
			kind: "tabs",
			placements,
			...(node.active && placements.includes(node.active)
				? { active: node.active }
				: {}),
		};
	}
...
function sanitizedLayout(
	layout: PageLayoutEnvelope,
	allowed: ReadonlySet<string>,
): PageLayoutEnvelope {
	return {
		version: layout.version,
		root: sanitizeLayoutNode(layout.root, allowed) ?? {
			kind: "tabs",
			placements: [],
		},
	};
...
function isPageLayoutEnvelope<LegacyLayout>(
	layout: PageTypeRegistration<LegacyLayout>["layout"],
): layout is PageLayoutEnvelope {
	return typeof layout === "object" && layout !== null && "version" in layout && "root" in layout;
}

export function createPageRegistry<LegacyLayout = unknown>(
	contributions: ContributionRegistry<PagePlatformContribution<LegacyLayout>>,
): PageRegistry<LegacyLayout> {
	let cache:
		| { readonly version: number; readonly snapshot: PageRegistrySnapshot<LegacyLayout> }
		| undefined;

	const derive = (): PageRegistrySnapshot<LegacyLayout> => {
		...
		const pages = new Map<
			QualifiedPageTypeId,
			Array<{ owner: string; definition: PageTypeRegistration<LegacyLayout> }>
		>();
		...
		const resolvedPages = new Map<QualifiedPageTypeId, ResolvedPageType<LegacyLayout>>();
		...
		const snapshot: PageRegistrySnapshot<LegacyLayout> = {
			generation: version,
	...
	return {
		...
		validateContribution(owner, contribution) {
			...
			for (const panel of contribution.panelTypes ?? []) {
				...
				if (
					(contribution.panelTypes ?? []).filter(
			...
			const knownPage = (id: QualifiedPageTypeId): boolean =>
				pageIds.has(id) || existing.pageTypes.has(id);
			const contributionIds = new Set<string>();
			for (const activity of contribution.activities ?? []) {
				if (ownerFromQualifiedId(activity.id) !== owner) {
					throw new PagePlatformError(
						"PAGE_CONTRIBUTION_OWNER_MISMATCH",
						`activity "${activity.id}" does not belong to "${owner}"`,
					);
				}
				if (contributionIds.has(activity.id)) {
					throw new PagePlatformError(
						"PAGE_CONTRIBUTION_CONFLICT",
						`activity "${activity.id}" is already registered`,
					);
				}
				contributionIds.add(activity.id);
				if (
					Number(Boolean(activity.pageTypeId)) +
						Number(Boolean(activity.commandId)) !==
					1
				) {
					throw new PagePlatformError(
						"PAGE_TYPE_UNAVAILABLE",
						`activity "${activity.id}" must have exactly one launch target`,
					);
				}
				if (activity.pageTypeId && !knownPage(activity.pageTypeId)) {
					throw new PagePlatformError(
						"PAGE_TYPE_UNAVAILABLE",
						`activity "${activity.id}" references missing page type "${activity.pageTypeId}"`,
					);
				}
			}
			for (const editor of contribution.resourceEditors ?? []) {
				if (ownerFromQualifiedId(editor.id) !== owner) {
					throw new PagePlatformError(
						"PAGE_CONTRIBUTION_OWNER_MISMATCH",
						`resource editor "${editor.id}" does not belong to "${owner}"`,
					);
				}
				if (contributionIds.has(editor.id)) {
					throw new PagePlatformError(
						"PAGE_CONTRIBUTION_CONFLICT",
						`resource editor "${editor.id}" is already registered`,
					);
				}
				...
				if (!knownPage(editor.pageTypeId)) {
					throw new PagePlatformError(
						"PAGE_TYPE_UNAVAILABLE",
						`resource editor "${editor.id}" references missing page type "${editor.pageTypeId}"`,
					);
				}
			}
		},
